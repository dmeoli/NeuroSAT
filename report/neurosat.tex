...
down from $\sim\!15$ for an untrained network. As with the graph models, this
decision count does not translate into wall-clock speed: each decision runs an MCTS
search ($\approx\!160$\,ms on CPU), so a solve takes $\approx\!0.9$\,s per instance
--- orders of magnitude slower than MiniSat's milliseconds, and heavier still than a
single GNN forward.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{mrir_time.png}
\caption{Iterations (left, MRIR) vs.\ wall-clock time (right, seconds-to-solve),
Graph-Q-SAT vs.\ GAT-Q-SAT, per regime (cap~500). Attention is slower per decision,
so fewer iterations $\neq$ less time.}
\label{fig:time}
\end{figure}
...
surveys~\cite{survey2025} catalogue the design space.
...
were designed to close. Pushing inference \emph{offline} (as NeuroBack does) for
genuine wall-clock gains remains promising future work.
