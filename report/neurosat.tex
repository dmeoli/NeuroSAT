...
\paragraph{Result.} After self-play training the policy solves the held-out
\texttt{uf20-91} test set in $\approx\!5.4$ mean branching decisions (best $3.4$),
down from $\sim\!15$ for an untrained network. A multi-head self-attention variant
of the CNN was tried but did \emph{not} improve on this baseline and was dropped.
...
\paragraph{GTv2-Q-SAT (this work).} Motivated by NeuroBack's graph-transformer
encoder~\cite{neuroback}, we add a third, more expressive attention variant: the two
static \texttt{GATConv} layers are replaced by a single \emph{pre-norm Transformer
block} built on \texttt{GATv2}~\cite{brody2022} --- which computes \emph{dynamic},
input-dependent edge-featured attention rather than GAT's static scoring --- with a
parallel feed-forward branch and a residual connection. It keeps the bipartite,
self-loop-free message passing and is selected at run time
(\texttt{--attention\_type graph\_transformer}), giving the model lineage
Graph-Q-SAT $\rightarrow$ GAT-Q-SAT $\rightarrow$ GTv2-Q-SAT.
...
\begin{figure}[h]
\centering
\begin{subfigure}{0.33\textwidth}\includegraphics[width=\textwidth]{coloring_indist.png}\end{subfigure}
\begin{subfigure}{0.33\textwidth}\includegraphics[width=\textwidth]{random_transfer.png}\end{subfigure}
\begin{subfigure}{0.33\textwidth}\includegraphics[width=\textwidth]{random_indist.png}\end{subfigure}
...
\paragraph{Cross-domain transfer.} Uniform-random 3-SAT lacks the structure of a
real application, so we further ask a sharper question: does the
\emph{colouring-trained} heuristic transfer, \emph{with no retraining}, to entirely
different structured domains? We evaluate the colouring-trained Graph-Q-SAT and
GAT-Q-SAT checkpoints on four unseen families from SATLIB: small-world graph
colouring (a topology shift within the training problem), AIM (controlled
structured 3-SAT), quasigroups (combinatorial design) and planning
(blocks-world/logistics). Figure~\ref{fig:transfer} reports the median MRIR
(cap~200). The attention model is the more \textbf{robust} transferrer: GAT-Q-SAT
stays at or above the MiniSat baseline (MRIR~$\ge 1$) on \textbf{three of the four}
domains (AIM~1.29, quasigroup~1.01, planning~1.14), whereas plain Graph-Q-SAT does
so on only \textbf{one} (AIM~1.50). On the genuinely different combinatorial and
application domains (quasigroup, planning) GAT-Q-SAT beats MiniSat while Graph-Q-SAT
drops below it; on AIM both transfer well (Graph marginally ahead) and on the
small-world topology shift both fall below~$1$. Graph attention thus improves not
only in-distribution performance but \emph{cross-domain generalisation}.
\begin{figure}[h]
\centering
\includegraphics[width=0.72\textwidth]{transfer.png}
\caption{Cross-domain transfer of the colouring-trained heuristic: median MRIR
(cap~200) on four unseen SATLIB families. GAT-Q-SAT stays $\ge$ MiniSat on 3/4
domains, Graph-Q-SAT on 1/4.}
\label{fig:transfer}
\end{figure}
We note this is a \emph{preliminary} study: AIM (72) and small-world (60) give
robust medians, but quasigroup (11) and planning (8) are small SATLIB families and
the numbers there are noisier; results are single-seed at cap~200. A larger panel
is left for future work.

\paragraph{Iterations vs.\ wall-clock time.} MRIR rewards \emph{fewer CDCL
iterations}, but each model decision is a full GNN forward pass, and attention
makes it costlier still --- so an iteration win need not be a time win.
Figure~\ref{fig:time} reports the median wall-clock seconds-to-solve (cap~500) per
regime. On in-distribution colouring GAT-Q-SAT matches Graph-Q-SAT's time
($1.1$\,s) while needing fewer iterations --- a genuine win; but on random 3-SAT
GAT-Q-SAT is $\sim\!2\times$ slower ($6.7$\,s vs $3.2$\,s) \emph{and} reduces
iterations less. In absolute terms the agents take $1$--$7$\,s per (tiny) instance,
which MiniSat solves in \emph{milliseconds}: at this scale the per-decision GNN cost
dominates and the iteration reduction does \textbf{not} translate into a wall-clock
speed-up. This is precisely the limitation the restricted heuristics (early release /
action pool / Q-value warm-start, and NeuroBack's offline inference) are designed
to remove.
\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{time.png}
\caption{Wall-clock companion to Figure~\ref{fig:thesis}: median seconds-to-solve
(cap~500). Attention is slower per decision; fewer iterations $\neq$ less time.}
\label{fig:time}
\end{figure}
...
\texttt{make\_plots.py}, \texttt{paper\_analysis.py} and \texttt{transfer\_study.py}
...
lines~\cite{mlforsat2023}, and benchmarks~\cite{g4satbench} and
surveys~\cite{survey2025} catalogue the design space. Our \emph{GTv2-Q-SAT} brings
the graph-transformer attention of this trajectory back into the \emph{online}
RL-branching setting of Graph-Q-SAT.
...
were designed to close. We realised one promising next step, \textbf{GTv2-Q-SAT}, a
GATv2 graph-transformer attention block inspired by NeuroBack; pushing inference
\emph{offline} for genuine wall-clock gains remains future work.
...
\bibitem{brody2022} S. Brody, U. Alon, E. Yahav. \emph{How Attentive are Graph
  Attention Networks?} ICLR 2022. arXiv:2105.14491.
...
\bibitem{mlforsat2023} N. Shirokikh et al. \emph{Machine Learning for SAT: Restricted
  Heuristics and New Graph Representations}. arXiv:2307.09141, 2023.
\bibitem{g4satbench} Z. Li, J. Guo, X. Si. \emph{G4SATBench: Benchmarking and
  Advancing SAT Solving with Graph Neural Networks}. arXiv:2309.16941, 2023.
