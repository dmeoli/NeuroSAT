...
ported to a single self-contained PyTorch stack, and the published results are
reproduced exactly.
...
**Findings:**
- **GAT-Q-SAT** (graph attention) does better than plain Graph-Q-SAT on structured
  problems (graph colouring), both in distribution and under transfer, with an
  advantage that grows with the size of the instances; it also transfers better to
  unseen domains (it stays ≥ MiniSat on 3/4 cross-domain families, against 1/4 for
  Graph-Q-SAT). On uniform-random 3-SAT plain Graph-Q-SAT is the stronger of the two.
- **Iterations against wall-clock:** fewer CDCL iterations (MRIR > 1) need not be
  less time, since each decision is a GNN forward pass and attention makes it
  costlier; the agents take ~1–7 s per problem against MiniSat's milliseconds, which
  is the point the restricted heuristics attack.
- **AlphaZeroSAT** learns a heuristic from self-play (~5.4 mean decisions), but each
  MCTS decision costs ~160 ms, hence ~0.9 s per problem (the same caveat), and the
...
The PyTorch models run on a current stack: the latest `torch` and
`torch-geometric` (no `torch-scatter`/`torch-sparse`), `numpy>=2`, `gymnasium`.
...
NeuroBack, …). This repository is the course project, and compares the two
approaches; the scientific note on the attention model alone lives with its
code, in [`GQSAT/report/gatqsat.tex`](GQSAT/report).
